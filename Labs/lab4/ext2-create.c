...
void write_block_bitmap(int fd)
{
	...
	//Initialize total bits as the block size * 8:
	const int TOTAL_BITS = BLOCK_SIZE * 8;

	//Assertion Check
	map_value[0] |= (1 << 0);
	assert((map_value[0] & (1 << 0)) != 0);
	...
	for(int i = SUPERBLOCK_BLOCKNO; i < LAST_BLOCK; i++)
	{
		...
		assert( (map_value[byte_offset] & (1 << bit_offset)) != 0);
	}
...
void write_root_dir_block(int fd)
{
	...
	//Struct for Lost and Found entry:
