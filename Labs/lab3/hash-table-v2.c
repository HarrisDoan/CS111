...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		//Mutex: Init
		int mutex_status = pthread_mutex_init(&entry->mutex, NULL);
                             ...
                             uint32_t value)
{
	...
	//After declaring the hash_table_entry struct, add the mutex lock and check

	//Mutex: Lock
	int mutex_status = pthread_mutex_lock(&hash_table_entry->mutex);
	...
	//Add else condiiton since we don't want this to always run
	else
	{
	...
	//Mutex: Unlock
	mutex_status = pthread_mutex_unlock(&hash_table_entry->mutex);
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		//Mutex: Destroy
		int mutex_status = pthread_mutex_destroy(&entry->mutex);
